...
const URL='http://127.0.0.1:4175/index.html?game-test=1&v=54&maps-v6=1';
...
async function main(){
  ...
  await c.send('Page.navigate',{url:URL});await until(c,'typeof rpgLaunch==="function"&&window.__RPG_V5__?.version===6');
  const results=[];
  for(let id=1;id<=5;id++){
    ...
    assert(Math.abs(data.bodyBottom-480)<=2&&data.frame>=0&&data.frame<=5,`stage ${id}: player is not grounded/idle (${data.bodyBottom}/${data.frame})`);
    results.push(data);
  }
