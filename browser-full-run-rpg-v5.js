...
const URL='http://127.0.0.1:4175/index.html?game-test=1&v=52&full-run-v5=1';
...
async function main(){
  ...
  await until(c,'typeof rpgLaunch==="function"&&window.__RPG_V5__?.version===5');
  ...
  for(const id of ids){
    ...
    await c.eval('__RPG_TEST__.soulCorrect()');await wait(1050);
  }
  ...
  await c.eval(`(()=>{const s=__RPG_TEST__.scene();s.player.setPosition(s.portal.x-80,420);return s.tryPortalStrike()})()`);
