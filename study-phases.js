...
home=function(){
  ...
  let nextAction={label:'开始今日基础训练',detail:`从第 ${baseDone+1} / ${baseTarget} 次提取开始`,onclick:"showView('practice')"};
  if(recall?.taskIds?.length&&!recall.done)nextAction={label:'先完成昨日默写回收',detail:`${recall.taskIds.length}题，写完后自动进入今日学习`,onclick:"openDailyDictation('recall')"};
  else if(!lesson.done)nextAction={label:baseDone?'继续今日基础训练':'开始今日基础训练',detail:`三轮共 ${baseTarget} 次提取，当前完成 ${baseDone} 次`,onclick:"showView('practice')"};
  else if(mode==='A+B'&&!sprint.done)nextAction={label:'开始今日错题追击',detail:`优先完成 ${sprint.size} 道高频与到期错题`,onclick:"showView('mistakeSprint')"};
  else if(mode==='C'&&!subjective.done)nextAction={label:'开始考场式微默写',detail:'关键词、完整句、简答骨架共3题',onclick:"showView('subjectivePractice')"};
  else if(mode!=='C'&&!todayDictation.done)nextAction={label:'开始今日重点默写',detail:`${todayDictation.taskIds.length}题，提交后对照遗漏核心点`,onclick:"openDailyDictation('today')"};
  else nextAction={label:'进入今日游戏关卡',detail:'今日学习已完成，关卡与进度均已解锁',onclick:'enterDailyGame()'};
  const tasks=[];
  ...
  return shell('今天，把答案真正写出来。',`DAY ${String(lesson.dayNumber).padStart(2,'0')} · ${phaseLabel(mode)}`,`<p class="lede">先回收昨天，再学习今天，最后通过默写确认自己真的能写出来。</p><div class="phase-banner mode-${mode.replace('+','b').toLowerCase()}"><div class="phase-banner-copy"><span>今日模式</span><h2>${phaseLabel(mode)}</h2><p>${stageText} · 预计 ${phaseEstimate(mode)}</p><button class="phase-primary-action" onclick="${nextAction.onclick}"><span>${nextAction.label}</span><b>→</b></button><small class="phase-primary-meta">${nextAction.detail}</small></div><div class="phase-days"><strong>${daysLeft()}</strong><span>天后考试</span></div></div><div class="stats"><button class="stat phase-stat-link" onclick="${recall&&!recall.done?"openDailyDictation('recall')":"showView('practice')"}"><span>基础训练</span><strong>${baseDone}<small> / ${baseTarget}次</small></strong><small>${lesson.done?'已完成':'点击进入提取练习'} →</small></button><button class="stat phase-stat-link" onclick="showView('mistakes')"><span>待掌握错题</span><strong>${phasePendingCount()}<small> 题</small></strong><small>${mode==='A'?'查看错题 · 达到15题开启B':'进入错题复习'} →</small></button><button class="stat phase-stat-link" onclick="${mode==='C'?"showView('subjectivePractice')":"openDailyDictation('today')"}" ${mode!=='C'&&!lesson.done?'disabled':''}><span>今日输出</span><strong>${mode==='C'?(subjective?.done?'完成':'3题'):(todayDictation?.done?'完成':`${todayDictation?.taskIds.length||0}题`)}</strong><small>${mode!=='C'&&!lesson.done?'完成基础训练后解锁':'点击进入默写'} →</small></button></div><div class="section-head"><div><h3>今日学习路线</h3><p>昨日回收 → 基础训练 → 今日默写 → 游戏关卡</p></div><button class="text-btn" onclick="showView('mistakes')">查看全部错题 →</button></div><div class="task-grid phase-task-grid">${tasks.join('')}</div>${typeof gameStatusCard==='function'?gameStatusCard():''}`);
};
